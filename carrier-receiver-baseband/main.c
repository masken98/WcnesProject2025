 ...
 */
 ...
 void decode_bch_in_c(uint8_t *received_data, size_t length) {
     // Create a string from received data
     char hex_string[1024] = {0};
     char temp[5];
 
     for (size_t i = 0; i < length; i++) {
         snprintf(temp, sizeof(temp), "%02X", received_data[i]);
     ...
     char command[2048];
     snprintf(command, sizeof(command), "python3 decode_bch.py \"%s\"", hex_string);
 
     printf("Running command: %s\n", command);
     int ret = system(command);
     if (ret != 0) {
         printf("Error: BCH decoding script failed!\n");
     }
 }
